...
const transactions = [
  { time: "14:20", type: "INCOME", cat: "Technical Service", method: "Card", amount: "₺1.250,00", status: "Approved" },
  { time: "13:45", type: "EXPENSE", cat: "Part Purchase", method: "Cash", amount: "- ₺3.400,00", status: "Paid", error: true },
  { time: "12:10", type: "EXPENSE", cat: "Rent", method: "Transfer", amount: "- ₺12.000,00", status: "Pending", error: true },
  { time: "10:30", type: "INCOME", cat: "Product Sale", method: "Card", amount: "₺550,00", status: "Approved" },
];
    ...
    <div className="p-8 space-y-8 bg-slate-50 dark:bg-slate-950 min-h-screen">
      <div className="flex justify-between items-center h-16 px-2 border-b border-slate-200 dark:border-slate-800 -mt-4 mb-4">
        <div className="flex items-center gap-6">
          <h1 className="text-xl font-black tracking-tighter text-slate-900 dark:text-white uppercase">Finance & Cash Management</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {[
          { label: "CASH BALANCE", val: "₺42.850,00", sub: "Main safe & drawer total", icon: Landmark, color: "text-blue-600", bg: "bg-blue-50 dark:bg-blue-900/20" },
          { label: "DAILY INCOME", val: "₺8.420,00", sub: "Total cash entry today", trend: "+12%", icon: TrendingUp, color: "text-emerald-600", bg: "bg-emerald-50 dark:bg-emerald-900/20" },
          { label: "PENDING POS", val: "₺15.200,45", sub: "Total in provision", icon: CreditCard, color: "text-blue-400", bg: "bg-blue-50 dark:bg-blue-900/20" },
          { label: "MONTHLY NET PROFIT", val: "₺112.900", sub: "After all expenses", trend: "+8.4%", icon: BarChart3, color: "text-emerald-500", bg: "bg-emerald-50 dark:bg-emerald-900/20" },
        ].map((s, i) => (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 bg-white dark:bg-slate-900 p-8 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-sm">
          <div className="flex justify-between items-center mb-10">
            <div>
              <h2 className="text-lg font-black tracking-tight text-slate-900 dark:text-white uppercase">Income vs Expense Trends</h2>
              <p className="text-xs font-bold text-slate-500 uppercase tracking-widest mt-1">Weekly financial performance analysis</p>
            </div>
            <div className="flex gap-2 bg-slate-100 dark:bg-slate-800 p-1 rounded-xl">
              <button className="px-4 py-1.5 text-[10px] font-black uppercase tracking-widest rounded-lg bg-white dark:bg-slate-700 text-slate-900 dark:text-white shadow-sm">Weekly</button>
              <button className="px-4 py-1.5 text-[10px] font-black uppercase tracking-widest rounded-lg text-slate-400 hover:text-slate-600 transition-colors">Monthly</button>
            </div>
          </div>

          <div className="h-64 flex items-end justify-between gap-6 px-4">
            {[
              { day: "Mon", income: 40, expense: 20 },
              { day: "Tue", income: 60, expense: 30 },
              { day: "Wed", income: 85, expense: 25, active: true },
              { day: "Thu", income: 55, expense: 45 },
              { day: "Fri", income: 70, expense: 30 },
              { day: "Sat", income: 30, expense: 15 },
              { day: "Sun", income: 10, expense: 5 },
            ].map((d, i) => (
          ...
          <div className="mt-10 flex gap-8 px-4">
            <div className="flex items-center gap-2">
              ...
              <span className="text-[10px] font-black text-slate-600 dark:text-slate-400 uppercase tracking-[0.2em]">Income</span>
            </div>
            <div className="flex items-center gap-2">
              <div className="w-3 h-3 rounded-full bg-red-400/50"></div>
              <span className="text-[10px] font-black text-slate-600 dark:text-slate-400 uppercase tracking-[0.2em]">Expense</span>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white dark:bg-slate-900 p-8 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-sm flex flex-col">
          <h2 className="text-lg font-black tracking-tight text-slate-900 dark:text-white uppercase mb-8 tracking-widest">Revenue Distribution</h2>
          <div className="flex-1 space-y-8">
            {[
              { label: "Technical Service", val: "₺24.500", p: "w-[55%]", color: "bg-blue-600" },
              { label: "Product Sales", val: "₺12.200", p: "w-[30%]", color: "bg-blue-400" },
              { label: "2nd Hand Trade", val: "₺6.150", p: "w-[15%]", color: "bg-emerald-500" },
            ].map((r, i) => (
          ...
          <div className="mt-10 p-5 bg-blue-50 dark:bg-blue-900/10 rounded-2xl border border-blue-100 dark:border-blue-800 flex items-center justify-between">
            <div className="flex flex-col">
              <span className="text-[9px] font-black text-blue-400 uppercase tracking-[0.2em]">Active Campaign</span>
              <span className="text-xs font-black text-blue-700 dark:text-blue-300 mt-0.5">Screen Repair 10% Discount</span>
            </div>
      ...
      <div className="bg-white dark:bg-slate-900 rounded-[2rem] shadow-sm border border-slate-200 dark:border-slate-800 overflow-hidden">
        <div className="p-8 border-b border-slate-100 dark:border-slate-800 flex justify-between items-center">
          <div>
            <h2 className="text-lg font-black tracking-tight text-slate-900 dark:text-white uppercase tracking-widest">Recent Transactions & Expenses</h2>
            <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest mt-1">All movements within the last 24 hours</p>
          </div>
        ...
        <div className="px-8 py-5 bg-slate-50 dark:bg-slate-950/50 border-t border-slate-100 dark:border-slate-800 flex justify-between items-center text-[10px] font-black text-slate-400 uppercase tracking-widest">
          <span>Total 48 records found</span>
          <div className="flex items-center gap-4">
            ...
            <span className="text-slate-900 dark:text-white">Page 1 / 5</span>
      ...
      <div className="fixed bottom-8 right-8 flex flex-col gap-4 z-50">
        <button className="group flex items-center gap-3 bg-white dark:bg-slate-900 text-slate-700 dark:text-slate-200 px-6 py-4 rounded-2xl shadow-2xl border border-slate-200 dark:border-slate-800 hover:bg-slate-50 transition-all active:scale-95">
          ...
          <span className="font-black text-[11px] uppercase tracking-widest">Cash In/Out</span>
        </button>
