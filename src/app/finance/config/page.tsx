      ...
      <div className="flex items-center justify-between mb-8 sticky top-0 z-40 bg-slate-950/80 backdrop-blur-xl py-4 border-b border-slate-800/50">
        <div className="flex items-center gap-4">
          ...
          <nav className="flex gap-4 text-[10px] font-black uppercase tracking-widest text-slate-500">
            ...
            <span className="text-blue-400">Ödemeler ve Ağ Geçidi</span>
          </nav>
        </div>
      </div>
          ...
          <div key={i} className={cn("bg-slate-900 p-6 rounded-3xl border-l-4 shadow-sm", s.color)}>
            ...
            {s.trend ? (
              <div className="text-[10px] text-emerald-400 mt-2 flex items-center gap-1 font-black">
                <Activity className="w-3 h-3" /> Geçen aya göre {s.trend}
              </div>
            ) : (
          ...
          <div className="bg-slate-900 rounded-[2.5rem] overflow-hidden shadow-2xl border border-slate-800">
            <div className="p-8 border-b border-slate-800 flex justify-between items-center bg-slate-900/50">
              <div>
                ...
                <p className="text-[10px] text-slate-500 uppercase tracking-[0.2em] font-black mt-1">Müşteri ödeme seçeneklerini aktif edin</p>
              </div>
            ...
            <div className="divide-y divide-slate-800/50">
              {[
                ...
                { label: "Banka Havalesi (EFT)", sub: "Manuel mutabakat gerektirir", icon: Landmark, color: "text-purple-500", bg: "bg-purple-500/10" },
              ].map((m, i) => (
              ...
              <table className="w-full text-left">
                <thead>
                  <tr className="bg-slate-950/50">
                    <th className="px-8 py-5 text-[10px] font-black uppercase tracking-widest text-slate-500">Plan Türü</th>
                    <th className="px-8 py-5 text-[10px] font-black uppercase tracking-widest text-slate-500">Oran</th>
                    <th className="px-8 py-5 text-[10px] font-black uppercase tracking-widest text-slate-500">Kod</th>
                    <th className="px-8 py-5 text-[10px] font-black uppercase tracking-widest text-slate-500">Durum</th>
        ...
        <div className="lg:col-span-4 space-y-8">
          ...
          <div className="bg-slate-900 rounded-[2rem] p-8 shadow-2xl border border-slate-800">
            ...
            <div className="space-y-4">
              {[
                ...
                { name: "Ön Büro", balance: "28.450 ₺", currency: "YALNIZCA TRY", color: "text-emerald-400" },
              ].map((d, i) => (
          ...
          <div className="bg-slate-900 rounded-[2rem] p-8 shadow-2xl border border-slate-800">
            ...
            <div className="space-y-6">
              {[
                { text: "Akbank POS terminali yazılımı Sistem tarafından güncellendi", time: "2 saat önce", color: "bg-blue-500" },
                { text: "Tüm kategorilere yeni Global KDV oranı %20 uygulandı", time: "Dün, 14:30", color: "bg-amber-500" },
                { text: "PayTR entegrasyonu başarıyla yeniden yetkilendirildi", time: "24 Eki 2023", color: "bg-emerald-500" },
              ].map((l, i) => (
            ...
            <button className="w-full mt-10 py-3 border border-slate-800 rounded-2xl text-[10px] font-black text-slate-500 hover:bg-slate-800 hover:text-white transition-all uppercase tracking-[0.2em] shadow-lg">Tüm Günlükleri Gör</button>
          </div>
        </div>
      </div>
    </div>
