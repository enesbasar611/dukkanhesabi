  ...
  return (
    <div className="p-8 max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-12 gap-8 pb-32">
      ...
      <div className="lg:col-span-8 space-y-8">
        <div className="mb-10">
          <h2 className="text-3xl font-black tracking-tighter text-slate-900 dark:text-white mb-2">Cash In/Out Operation</h2>
          <p className="text-slate-500 font-bold uppercase text-[10px] tracking-widest">Record workshop financial movements with precision.</p>
        </div>
        ...
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-[2.5rem] overflow-hidden shadow-2xl">
          <div className="p-10 space-y-10">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
              ...
              <div className="space-y-3">
                <label className="block text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 ml-1">Operation Category</label>
              ...
              {/* Amount */}
              <div className="space-y-3">
                <label className="block text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 ml-1">Amount</label>
              ...
              {/* Method */}
              <div className="space-y-3">
                <label className="block text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 ml-1">Payment Method</label>
              ...
              {/* Drawer */}
              <div className="space-y-3">
                <label className="block text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 ml-1">Account / Drawer</label>
                <select className="w-full bg-slate-50 dark:bg-slate-950 border-none ring-1 ring-slate-200 dark:ring-slate-800 focus:ring-2 focus:ring-blue-600 rounded-2xl py-4 px-5 text-sm font-bold text-slate-700 dark:text-slate-200 outline-none transition-all">
                  <option selected>Main Safe</option>
                  <option>Front Desk Drawer</option>
                </select>
              </div>

              {/* Date */}
              <div className="space-y-3 md:col-span-2">
                <label className="block text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 ml-1">Date & Time</label>
              ...
              {/* Desc */}
              <div className="space-y-3 md:col-span-2">
                <label className="block text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 ml-1">Description / Internal Notes</label>
                <textarea className="w-full bg-slate-50 dark:bg-slate-950 border-none ring-1 ring-slate-200 dark:ring-slate-800 focus:ring-2 focus:ring-blue-600 rounded-2xl py-4 px-5 text-sm font-bold text-slate-700 dark:text-slate-200 outline-none transition-all min-h-[100px] resize-none" placeholder="Enter transaction details..." rows={3}></textarea>
              </div>
            </div>

            {/* Document Upload */}
            <div className="space-y-3">
              <label className="block text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 ml-1">Attachment / Receipt</label>
              <div className="border-4 border-dashed border-slate-100 dark:border-slate-800/50 rounded-3xl p-12 flex flex-col items-center justify-center hover:border-blue-500/20 hover:bg-blue-50/30 dark:hover:bg-blue-900/5 transition-all cursor-pointer group">
                ...
                <p className="text-sm font-black text-slate-900 dark:text-white uppercase tracking-widest">Drop file or <span className="text-blue-600">browse</span></p>
                <p className="text-[9px] text-slate-400 mt-2 font-bold uppercase tracking-widest">PNG, JPG, PDF (Max 10MB)</p>
              </div>
            </div>
          </div>
          ...
          <div className="bg-slate-50 dark:bg-slate-950/50 p-8 flex items-center justify-end gap-6 border-t border-slate-100 dark:border-slate-800">
            <button className="px-8 py-3 text-xs font-black uppercase tracking-[0.2em] text-slate-400 hover:text-slate-900 dark:hover:text-white transition-all">Cancel</button>
      ...
      <div className="lg:col-span-4 space-y-8">
        <div className="space-y-4">
          <h3 className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 px-1">Live Account Balances</h3>
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-8 rounded-3xl shadow-sm relative overflow-hidden group">
            ...
            <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Main Safe</p>
          ...
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-8 rounded-3xl shadow-sm">
            <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Front Desk</p>
        ...
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl shadow-sm overflow-hidden flex flex-col">
          <div className="p-6 border-b border-slate-100 dark:border-slate-800 flex justify-between items-center bg-slate-50 dark:bg-slate-950/30">
            <h3 className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-900 dark:text-white">Recent Movements</h3>
            <History className="w-4 h-4 text-slate-400" />
          </div>
          <div className="divide-y divide-slate-50 dark:divide-slate-800">
            {[
              { label: "Screen Repair", sub: "14:20 • Desk", val: "+₺1.450", color: "text-emerald-500", in: true },
              { label: "Lunch Expense", sub: "12:15 • Safe", val: "-₺240", color: "text-red-500", in: false },
              { label: "iPhone 14 Sale", sub: "11:05 • Safe", val: "+₺32.000", color: "text-emerald-500", in: true },
              { label: "Courier Fee", sub: "09:30 • Desk", val: "-₺85", color: "text-red-500", in: false },
            ].map((m, i) => (
          ...
          <button className="p-4 text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 hover:text-blue-600 transition-colors border-t border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-950/30">View All Movements</button>
        </div>
      </div>
    </div>
  );
