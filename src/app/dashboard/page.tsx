...
async function getDashboardData() {
  const [
    ...
    prisma.serviceTicket.count({ where: { status: { in: ['NEW', 'REPAIRING'] } } }),
    prisma.product.count({ where: { stock: { lt: 5 } } }),
    prisma.cashDrawer.aggregate({ _sum: { balance: true } }),
    prisma.transaction.findMany({ take: 3, orderBy: { createdAt: 'desc' }, include: { customer: true } }),
    prisma.serviceTicket.findMany({ take: 3, orderBy: { createdAt: 'desc' }, include: { customer: true, technician: true } }),
    prisma.product.findMany({ take: 4, orderBy: { stock: 'desc' } })
  ]);
...
export default async function MainDashboard() {
  ...
  const metrics = [
    { label: "Bugünkü Satışlar", val: "1.250,00 ₺", trend: "+12%", icon: DollarSign, color: "text-emerald-600", bg: "bg-emerald-50 dark:bg-emerald-900/20" },
    { label: "Tamir Geliri", val: "850,00 ₺", trend: "+8%", icon: Wrench, color: "text-blue-600", bg: "bg-blue-50 dark:bg-blue-900/20" },
    { label: "Ödemeler", val: "2.100,00 ₺", icon: Wallet, color: "text-purple-600", bg: "bg-purple-50 dark:bg-purple-900/20" },
    { label: "Bekleyen", val: data.activeTickets.toString(), tag: "Acil", icon: Clock, color: "text-orange-600", bg: "bg-orange-50 dark:bg-orange-900/20" },
    { label: "Müşteriler", val: data.totalCustomers.toString(), icon: CheckCircle2, color: "text-emerald-600", bg: "bg-emerald-50 dark:bg-emerald-900/20" },
    { label: "Düşük Stok", val: data.lowStockCount.toString(), tag: "Az", icon: AlertTriangle, color: "text-red-600", bg: "bg-red-50 dark:bg-red-900/20" },
    { label: "Toplam Borçlar", val: "450,00 ₺", icon: CreditCard, color: "text-red-600", bg: "bg-red-50 dark:bg-red-900/20" },
    { label: "Kasa Bakiyesi", val: `${data.totalBalance.toLocaleString('tr-TR')} ₺`, icon: Landmark, color: "text-blue-600", bg: "bg-blue-50 dark:bg-blue-900/20", hero: true },
  ];
  ...
  return (
    <div className="p-8 space-y-8 bg-slate-50 dark:bg-slate-950 min-h-screen pb-20">
          ...
          <div key={i} className={cn(
            ...
            <div className="flex justify-between items-start mb-6">
              ...
              {m.trend ? (
                ...
                <span className={cn("text-[10px] font-black px-2 py-1 rounded-lg uppercase tracking-widest", m.tag === 'Acil' ? "bg-red-50 text-red-600" : "bg-orange-50 text-orange-600")}>{m.tag}</span>
              ) : null}
            </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 bg-white dark:bg-slate-900 p-8 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-sm">
          <div className="flex justify-between items-center mb-10">
            <div>
              <h4 className="text-lg font-black tracking-tight text-slate-900 dark:text-white uppercase tracking-widest">Gelir Analizi</h4>
          ...
          <div className="flex justify-between mt-6 text-[10px] text-slate-400 font-black uppercase tracking-[0.3em] px-2">
            <span>Pzt</span><span>Sal</span><span>Çar</span><span>Per</span><span>Cum</span><span>Cmt</span><span className="text-blue-600">Bugün</span>
          </div>
        </div>

        {/* Servis Dağılımı */}
        <div className="bg-white dark:bg-slate-900 p-8 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-sm">
          <h4 className="text-lg font-black tracking-tight text-slate-900 dark:text-white uppercase tracking-widest">Servis Durumu</h4>
          ...
          <div className="relative w-52 h-52 mx-auto mb-10 group cursor-pointer">
            ...
            <div className="absolute inset-0 flex flex-col items-center justify-center">
              ...
              <span className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">Toplam Adet</span>
            </div>
          </div>
          <div className="space-y-4">
            {[
              ...
              { label: "Devam Ediyor", val: 8, color: "bg-orange-400" },
              { label: "Yeni", val: 9, color: "bg-slate-300" },
            ].map((s, i) => (
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-8">
        ...
        <div className="bg-white dark:bg-slate-900 rounded-3xl shadow-sm border border-slate-200 dark:border-slate-800 overflow-hidden flex flex-col">
          <div className="p-8 border-b border-slate-100 dark:border-slate-800 flex justify-between items-center bg-slate-50 dark:bg-slate-950/30">
            <h4 className="font-black text-slate-900 dark:text-white uppercase tracking-widest">Son İşlemler</h4>
            <button className="text-blue-600 text-[10px] font-black uppercase tracking-widest hover:underline decoration-2">Tümünü Gör</button>
          </div>
                ...
                {data.recentTransactions.map((t, i) => (
                  <tr key={i} className="hover:bg-slate-50/50 dark:hover:bg-slate-800/50 transition-colors group">
                    <td className="px-8 py-6">
                      <p className="text-sm font-black text-slate-900 dark:text-white">{t.customer?.name || "Genel"}</p>
                      <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest">{t.createdAt.toLocaleDateString('tr-TR')}</p>
                    </td>
                    <td className="px-8 py-6 text-xs font-bold text-slate-500 uppercase">{t.category}</td>
                    <td className="px-8 py-6 text-sm font-black text-slate-900 dark:text-white tracking-tighter">{t.amount.toLocaleString('tr-TR')} ₺</td>
                    <td className="px-8 py-6">
                      <span className={cn(
                        "px-3 py-1 rounded-lg text-[10px] font-black uppercase tracking-widest shadow-inner border",
                        t.type === 'INCOME' ? "bg-emerald-50 text-emerald-600 border-emerald-100" : "bg-blue-50 text-blue-600 border-blue-100"
                      )}>{t.type === 'INCOME' ? 'Gelir' : 'Gider'}</span>
                    </td>
                  </tr>
                ))}
              </tbody>
        ...
        <div className="bg-white dark:bg-slate-900 rounded-3xl shadow-sm border border-slate-200 dark:border-slate-800 overflow-hidden flex flex-col">
          <div className="p-8 border-b border-slate-100 dark:border-slate-800 flex justify-between items-center bg-slate-50 dark:bg-slate-950/30">
            <h4 className="font-black text-slate-900 dark:text-white uppercase tracking-widest">Son Servis Kayıtları</h4>
            <button className="text-blue-600 text-[10px] font-black uppercase tracking-widest hover:underline decoration-2">Tüm Fişler</button>
          </div>
          <div className="divide-y divide-slate-100 dark:divide-slate-800">
            {data.recentTickets.map((r, i) => (
              <div key={i} className="p-8 flex items-center justify-between hover:bg-slate-50/50 dark:hover:bg-slate-800/50 transition-colors">
                <div className="flex items-center gap-6">
                  ...
                  <div>
                    <h5 className="text-sm font-black text-slate-900 dark:text-white">{r.deviceBrand} {r.deviceModel}</h5>
                    <p className="text-[10px] text-slate-500 font-bold uppercase tracking-widest mt-1">
                      {r.customer.name} • <span className="text-orange-600 dark:text-orange-400 font-black truncate max-w-[150px] inline-block align-bottom">{r.problemDesc}</span>
                    </p>
                  </div>
                </div>
                <div className="text-right">
                  <span className={cn(
                    ...
                    r.status === 'REPAIRING' ? "bg-orange-50 text-orange-600 border-orange-100" :
                    "bg-slate-100 text-slate-600 border-slate-200"
                  )}>{r.status}</span>
                  <p className="text-[9px] text-slate-400 font-black uppercase tracking-tighter">Teknisyen: {r.technician?.name || 'Atanmadı'}</p>
                </div>
              </div>
            ))}
          </div>
      ...
      {/* En Çok Satan Envanter */}
      <div className="bg-white dark:bg-slate-900 p-8 rounded-[2.5rem] shadow-sm border border-slate-200 dark:border-slate-800">
        <div className="flex justify-between items-center mb-10">
          <div>
            <h4 className="text-lg font-black tracking-tight text-slate-900 dark:text-white uppercase tracking-widest">Öne Çıkan Ürünler</h4>
            <p className="text-xs font-bold text-slate-500 uppercase tracking-widest mt-1">Stoktaki popüler parçalar</p>
          </div>
        </div>
          ...
          {data.topProducts.map((p, i) => (
            <div key={i} className="p-5 rounded-3xl border border-slate-100 dark:border-slate-800 hover:shadow-2xl transition-all group hover:-translate-y-1">
              <div className="aspect-square mb-6 rounded-2xl bg-slate-50 dark:bg-slate-950 overflow-hidden flex items-center justify-center relative shadow-inner">
                <div className="w-12 h-12 text-slate-200"><Package className="w-full h-full" /></div>
                <div className="absolute top-3 right-3 bg-blue-600 text-white text-[9px] font-black px-2 py-1 rounded-lg shadow-lg uppercase tracking-widest">Stokta</div>
              </div>
              <h6 className="text-sm font-black text-slate-900 dark:text-white leading-tight mb-1">{p.name}</h6>
              <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-4">{p.category} • {p.stock} Adet</p>
              <div className="flex items-center justify-between">
                <span className="text-lg font-black text-blue-600 tracking-tighter">{p.salePrice.toLocaleString('tr-TR')} ₺</span>
