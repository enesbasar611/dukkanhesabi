  ...
  return (
    <div className="p-8 space-y-8 bg-slate-950 min-h-screen text-slate-100 pb-32">
      <div className="flex items-end justify-between">
        <div>
          <h1 className="text-3xl font-black tracking-tighter text-white">Depolama Birimleri</h1>
          <p className="text-slate-500 mt-1">Fiziksel donanım depolama bölgelerinin etkileşimli haritası.</p>
        </div>
        <div className="flex items-center gap-3">
          <div className="flex items-center gap-1 bg-slate-900 p-1 rounded-xl border border-slate-800 shadow-xl">
            <button className="px-5 py-2 text-xs font-black bg-blue-600 text-white rounded-lg shadow-sm uppercase tracking-tighter">Depo</button>
            <button className="px-5 py-2 text-xs font-bold text-slate-500 hover:text-slate-300 uppercase tracking-tighter transition-colors">Ön Büro</button>
            <button className="px-5 py-2 text-xs font-bold text-slate-500 hover:text-slate-300 uppercase tracking-tighter transition-colors">Arka Oda</button>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        {[
          ...
          { label: "Devir Hızı", value: "4.2", sub: "ort. gün", icon: Zap, color: "text-emerald-500" },
        ].map((stat, i) => (
            ...
            <div className="bg-slate-900/40 border border-slate-800 rounded-3xl p-6 shadow-2xl">
              <div className="flex justify-between items-start mb-8">
                <div>
                  ...
                  <p className="text-[10px] text-slate-500 uppercase font-black tracking-[0.2em]">Batarya & Güç</p>
                </div>
            ...
            <div className="p-6 flex items-center justify-between border-b border-slate-800 bg-slate-900/20">
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-[10px] text-slate-500 uppercase font-black tracking-widest mt-0.5">Ekran Grubu Depolama</p>
                </div>
              </div>
              <div className="flex items-center gap-3">
                ...
                <button className="flex items-center gap-2 px-5 py-2 text-xs font-black text-blue-400 bg-blue-600/10 rounded-xl hover:bg-blue-600/20 transition-all border border-blue-500/20 uppercase tracking-tighter">
                  <ArrowLeftRight className="w-4 h-4" /> Eşyaları Taşı
                </button>
              </div>
            </div>
          ...
          <div className="bg-gradient-to-br from-blue-900/60 to-slate-900 border border-blue-500/20 rounded-3xl p-8 relative overflow-hidden shadow-2xl">
            <div className="relative z-10">
              <div className="flex items-center gap-2 mb-4 text-blue-400">
                ...
                <h3 className="font-black tracking-tight text-white">Akıllı Stok</h3>
              </div>
              <p className="text-xs text-blue-200/70 mb-8 leading-relaxed font-medium">Circuit Pro AI, son onarım hacmine dayanarak kritik seviyelere yaklaşan 4 kalem tespit etti.</p>
              <div className="space-y-4 mb-8">
                {[
                  { name: "iP12 Batarya", eta: "2 gün kaldı" },
                  { name: "S21 Ekran Kiti", eta: "Kritik: 2 kaldı", danger: true }
                ].map((r, i) => (
                  <div key={i} className="flex items-center justify-between p-4 rounded-2xl bg-white/5 border border-white/5 hover:bg-white/10 transition-colors cursor-pointer group">
                    ...
                    <button className="text-[10px] font-black text-blue-400 uppercase tracking-widest opacity-0 group-hover:opacity-100 transition-opacity">Sipariş Ver</button>
                  </div>
                ))}
              </div>
