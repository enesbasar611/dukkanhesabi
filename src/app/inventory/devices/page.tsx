    ...
    <div className="p-8 space-y-8 bg-slate-50 dark:bg-slate-950 min-h-screen">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        {[
          ...
          { label: "Beklenen Kâr", value: "156.200 ₺", icon: TrendingUp, color: "text-emerald-600", bg: "bg-emerald-50 dark:bg-emerald-900/20" },
          { label: "Aylık Satış", value: "28", icon: ShoppingBag, color: "text-orange-600", bg: "bg-orange-50 dark:bg-orange-900/20" },
        ].map((stat, i) => (
      ...
      <div className="bg-slate-100 dark:bg-slate-900 p-6 rounded-2xl space-y-6 border border-slate-200 dark:border-slate-800">
        <div className="flex items-center justify-between">
          <h4 className="text-xs font-black uppercase tracking-widest text-slate-500">Stok Filtreleri</h4>
        ...
        <div className="grid grid-cols-1 md:grid-cols-12 gap-4">
          ...
          <div className="md:col-span-3">
            <select className="w-full bg-white dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl text-sm h-11 px-4 outline-none">
              <option>Cihaz Türü (Sıfır / 2. El)</option>
              <option>Sıfır</option>
              <option>2. El</option>
            </select>
          </div>
      ...
      <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-sm overflow-hidden border border-slate-200 dark:border-slate-800">
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-slate-50 dark:bg-slate-950/50">
                ...
                <th className="px-6 py-4 text-[10px] font-black text-slate-500 uppercase tracking-widest border-b border-slate-100 dark:border-slate-800">Tür / Durum</th>
                ...
                <th className="px-6 py-4 text-[10px] font-black text-slate-500 uppercase tracking-widest border-b border-slate-100 dark:border-slate-800 text-right">İşlem</th>
              </tr>
            </thead>
                ...
                <tr key={i} className="hover:bg-slate-50/30 dark:hover:bg-slate-800/30 transition-colors group">
                  ...
                  <td className="px-6 py-5">
                    <div className="space-y-1.5">
                      <span className={cn(
                        ...
                        device.type === 'SIFIR' ? "bg-blue-600 text-white" : "bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300"
                      )}>{device.type}</span>
                      <div className="flex items-center gap-1 text-[11px] font-bold text-slate-500">
                        {device.type === 'SIFIR' ? <CheckCircle className="w-3 h-3 text-blue-500" /> : <Battery className="w-3 h-3 text-orange-500" />}
                        {device.condition}
                      </div>
                    </div>
                  </td>
                    ...
                    <div className="w-48">
                      <div className="flex justify-between items-center mb-1.5">
                        <span className={cn(
                          ...
                          device.warranty === 'Bitti' ? "text-red-600" : "text-blue-600"
                        )}>
                      ...
                      <div className="h-1.5 w-full bg-slate-100 dark:bg-slate-800 rounded-full overflow-hidden shadow-inner">
                        <div
                          className={cn("h-full rounded-full transition-all duration-500", device.warranty === 'Bitti' ? "bg-red-500" : "bg-blue-500")}
                  ...
                  <td className="px-6 py-5 text-right">
                    <div className="flex justify-end gap-2">
                      ...
                      <button className="bg-blue-600 text-white rounded-xl shadow-sm hover:bg-blue-700 transition-all px-6 py-1.5 text-[10px] font-black uppercase tracking-widest active:scale-95">Satış Yap</button>
                    </div>
                  </td>
                </tr>
              ))}
        ...
        <div className="px-6 py-4 bg-slate-50 dark:bg-slate-950/50 border-t border-slate-100 dark:border-slate-800 flex items-center justify-between">
          ...
          <div className="flex gap-1.5">
            <button className="px-4 py-1.5 border border-slate-200 dark:border-slate-800 rounded-lg hover:bg-white dark:hover:bg-slate-800 text-[10px] font-black uppercase tracking-widest transition-all">Geri</button>
            <button className="w-8 h-8 bg-blue-600 text-white rounded-lg text-[10px] font-black shadow-md shadow-blue-200">1</button>
            <button className="px-4 py-1.5 border border-slate-200 dark:border-slate-800 rounded-lg hover:bg-white dark:hover:bg-slate-800 text-[10px] font-black uppercase tracking-widest transition-all">İleri</button>
          </div>
        </div>
      </div>
    </div>
