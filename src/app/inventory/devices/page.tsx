import { Search, Filter, Smartphone, Package, TrendingUp, ShoppingBag, Eye,   CheckCircle,  ShieldCheck, Battery } from "lucide-react";
...
const devices = [
  {
    ...
    warranty: "23 Months 12 Days Left",
    warrantyProgress: 95,
    price: "₺82.900",
    cost: "₺74.500",
    status: "In Stock"
  ...
  },
  {
    ...
    type: "USED",
    condition: "92% Battery",
    warranty: "3 Months 4 Days Left",
    warrantyProgress: 15,
    price: "₺36.000",
    cost: "₺31.500",
    status: "In Stock"
    ...
    <div className="p-8 space-y-8 bg-slate-50 dark:bg-slate-950 min-h-screen">
      <div className="flex items-end justify-between">
        <div>
          ...
          <h1 className="text-3xl font-extrabold text-slate-900 dark:text-white tracking-tight">Device Inventory Management</h1>
        </div>
        <div className="flex gap-3">
          ...
          <button className="flex items-center gap-2 px-6 py-2.5 bg-blue-600 text-white rounded-xl font-bold text-sm shadow-lg shadow-blue-500/20 transition-all active:scale-95">
            <Smartphone className="w-4 h-4" /> Add New Device
          </button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        {[
          { label: "In-Stock Devices", value: "142", trend: "+12%", icon: Package, color: "text-blue-600", bg: "bg-blue-50 dark:bg-blue-900/20" },
          { label: "Total Investment", value: "₺842.500", icon: CreditCard, color: "text-blue-600", bg: "bg-blue-50 dark:bg-blue-900/20" },
          { label: "Expected Profit", value: "₺156.200", icon: TrendingUp, color: "text-emerald-600", bg: "bg-emerald-50 dark:bg-emerald-900/20" },
          { label: "Monthly Sales", value: "28", icon: ShoppingBag, color: "text-orange-600", bg: "bg-orange-50 dark:bg-orange-900/20" },
        ].map((stat, i) => (
      ...
      <div className="bg-slate-100 dark:bg-slate-900 p-6 rounded-2xl space-y-6 border border-slate-200 dark:border-slate-800">
        <div className="flex items-center justify-between">
          <h4 className="text-xs font-black uppercase tracking-widest text-slate-500">Inventory Filters</h4>
        ...
        <div className="grid grid-cols-1 md:grid-cols-12 gap-4">
          <div className="md:col-span-12 relative group">
            ...
            <input className="w-full pl-12 pr-4 py-3 bg-white dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl text-sm focus:ring-2 focus:ring-blue-500/20 transition-all outline-none" placeholder="IMEI, Model or Serial Number..." type="text" />
          </div>
          <div className="md:col-span-3">
            <select className="w-full bg-white dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl text-sm h-11 px-4 outline-none">
              <option>Device Type (New / Used)</option>
              <option>New</option>
              <option>Used</option>
            </select>
          </div>
          ...
          <div className="md:col-span-3">
            <button className="w-full h-11 bg-blue-600 text-white rounded-xl font-bold text-sm shadow-md active:scale-95 transition-all uppercase tracking-widest">Apply Search</button>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-sm overflow-hidden border border-slate-200 dark:border-slate-800">
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-slate-50 dark:bg-slate-950/50">
                <th className="px-6 py-4 text-[10px] font-black text-slate-500 uppercase tracking-widest border-b border-slate-100 dark:border-slate-800">Device / IMEI</th>
                <th className="px-6 py-4 text-[10px] font-black text-slate-500 uppercase tracking-widest border-b border-slate-100 dark:border-slate-800">Type / Condition</th>
                <th className="px-6 py-4 text-[10px] font-black text-slate-500 uppercase tracking-widest border-b border-slate-100 dark:border-slate-800">Warranty Status</th>
                <th className="px-6 py-4 text-[10px] font-black text-slate-500 uppercase tracking-widest border-b border-slate-100 dark:border-slate-800">Pricing</th>
                ...
                <th className="px-6 py-4 text-[10px] font-black text-slate-500 uppercase tracking-widest border-b border-slate-100 dark:border-slate-800 text-right">Action</th>
              </tr>
            </thead>
                ...
                <tr key={i} className="hover:bg-slate-50/30 dark:hover:bg-slate-800/30 transition-colors group">
                    ...
                    <div className="w-48">
                      <div className="flex justify-between items-center mb-1.5">
                        <span className={cn(
                          ...
                          device.warranty === 'Expired' ? "text-red-600" : "text-blue-600"
                        )}>
                      ...
                      <div className="h-1.5 w-full bg-slate-100 dark:bg-slate-800 rounded-full overflow-hidden shadow-inner">
                        <div
                          className={cn("h-full rounded-full transition-all duration-500", device.warranty === 'Expired' ? "bg-red-500" : "bg-blue-500")}
                  ...
                  <td className="px-6 py-5">
                    <div className="text-sm">
                      ...
                      <p className="text-[10px] font-bold text-slate-500 uppercase mt-0.5">Cost: {device.cost}</p>
                    </div>
                  </td>
                  <td className="px-6 py-5">
                    <span className={cn(
                  ...
                  <td className="px-6 py-5 text-right">
                    <div className="flex justify-end gap-2">
                      ...
                      <button className="bg-blue-600 text-white rounded-xl shadow-sm hover:bg-blue-700 transition-all px-6 py-1.5 text-[10px] font-black uppercase tracking-widest active:scale-95">Sell</button>
                    </div>
                  </td>
                </tr>
              ))}
        ...
        <div className="px-6 py-4 bg-slate-50 dark:bg-slate-950/50 border-t border-slate-100 dark:border-slate-800 flex items-center justify-between">
          <p className="text-xs font-bold text-slate-500 uppercase tracking-widest">Showing 1-3 of 142 devices</p>
          <div className="flex gap-1.5">
            <button className="px-4 py-1.5 border border-slate-200 dark:border-slate-800 rounded-lg hover:bg-white dark:hover:bg-slate-800 text-[10px] font-black uppercase tracking-widest transition-all">Prev</button>
            <button className="w-8 h-8 bg-blue-600 text-white rounded-lg text-[10px] font-black shadow-md shadow-blue-200">1</button>
            <button className="px-4 py-1.5 border border-slate-200 dark:border-slate-800 rounded-lg hover:bg-white dark:hover:bg-slate-800 text-[10px] font-black uppercase tracking-widest transition-all">Next</button>
          </div>
        </div>
      </div>
    </div>
