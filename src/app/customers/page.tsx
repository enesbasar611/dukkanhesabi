...
async function getCustomersData() {
  const [customers, stats] = await Promise.all([
    prisma.customer.findMany({
      ...
      include: {
        ...
        debts: { select: { amount: true, status: true } }
      }
    }),
    {
      total: await prisma.customer.count(),
      vip: await prisma.customer.count({ where: { isVip: true } }),
      newThisMonth: await prisma.customer.count({
        where: { createdAt: { gte: new Date(new Date().getFullYear(), new Date().getMonth(), 1) } }
      }),
      blacklisted: 0 // No blacklist field yet, keeping at 0
    }
  ]);
  ...
  return {
    customers: customers.map(c => ({
      ...c,
      totalSpend: c.transactions.reduce((acc, t) => acc + t.amount, 0),
      totalDebt: c.debts.filter(d => d.status === 'UNPAID').reduce((acc, d) => acc + d.amount, 0)
    })),
    stats
  };
}
  ...
  return (
    <div className="p-8 space-y-8 bg-slate-50 dark:bg-slate-950 min-h-screen">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        {[
          { label: "Toplam Müşteri", value: stats.total.toString(), icon: Users, color: "text-blue-600", bg: "bg-blue-100" },
          { label: "VIP Müşteriler", value: stats.vip.toString(), icon: ShieldCheck, color: "text-emerald-600", bg: "bg-emerald-100" },
          { label: "Yeni (Bu Ay)", value: `+${stats.newThisMonth}`, icon: UserPlus, color: "text-purple-600", bg: "bg-purple-100" },
          { label: "Kara Liste", value: stats.blacklisted.toString(), icon: Ban, color: "text-red-600", bg: "bg-red-100" },
        ].map((stat, i) => (
      ...
      <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-sm overflow-hidden border border-slate-200 dark:border-slate-800">
          ...
          <table className="w-full text-left">
            <thead>
              <tr className="bg-slate-50 dark:bg-slate-950/50">
                ...
                <th className="px-6 py-4 text-xs font-bold uppercase tracking-wider text-slate-500 text-right">Toplam Harcama</th>
                <th className="px-6 py-4 text-xs font-bold uppercase tracking-wider text-slate-500 text-right">Mevcut Borç</th>
            ...
            <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                  ...
                  <td className="px-6 py-5">
                    <div className="flex flex-col">
                      ...
                      <div className="flex items-center gap-3 mt-1">
                        ...
                        <span className="text-xs text-slate-500 flex items-center gap-1"><Mail className="w-3 h-3" /> {c.email || 'E-posta yok'}</span>
                      </div>
                    </div>
                  </td>
                  <td className="px-6 py-5">
                    <span className={cn(
                      ...
                      c.isVip ? "bg-blue-100 text-blue-700" : "bg-slate-100 text-slate-600"
                    )}>{c.isVip ? 'VIP' : 'Standart'}</span>
                  </td>
                  <td className="px-6 py-5 text-right font-bold text-slate-900 dark:text-white text-sm">
                    {c.totalSpend.toLocaleString('tr-TR')} ₺
                  </td>
                  <td className="px-6 py-5 text-right">
                    <span className={cn(
                      "text-sm font-bold",
                      c.totalDebt > 0 ? "text-red-500" : "text-slate-400"
                    )}>{c.totalDebt.toLocaleString('tr-TR')} ₺</span>
                  </td>
                  <td className="px-6 py-5 text-right">
                    <div className="flex items-center justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                      <Link href={`/customers/${c.id}`} className="p-2 hover:bg-blue-50 dark:hover:bg-blue-900/30 text-blue-600 rounded-lg text-xs font-bold uppercase">
                        Detay
                      </Link>
              ...
              {customers.length === 0 && (
                <tr>
                  <td colSpan={5} className="px-6 py-10 text-center text-slate-400 italic">Henüz müşteri kaydı bulunmuyor.</td>
                </tr>
              )}
            </tbody>
          </table>
        ...
        <div className="px-6 py-4 bg-slate-50 dark:bg-slate-950/50 border-t border-slate-100 dark:border-slate-800 flex items-center justify-between">
          <span className="text-sm text-slate-500">{stats.total} kayıttan {customers.length} tanesi gösteriliyor.</span>
