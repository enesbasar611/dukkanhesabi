  ...
  return (
    <div className="p-8 space-y-8 bg-slate-950 min-h-screen text-slate-200 antialiased overflow-hidden">
      ...
      <section className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div>
          <p className="text-blue-500 font-black tracking-[0.2em] text-[10px] uppercase mb-1">Financial Performance</p>
          <h2 className="text-4xl font-black tracking-tight text-white uppercase leading-none">Detailed Sales Report</h2>
          <p className="text-slate-500 font-bold uppercase tracking-widest text-[10px] mt-2">Precision transaction and volume analysis</p>
        </div>

        {/* Global Filter Toolbar */}
        <div className="flex flex-wrap items-center gap-3">
          <FilterButton icon={Calendar} label="This Month" />
          <FilterButton icon={Box} label="All Categories" />
          <FilterButton icon={BadgeCheck} label="Main Branch" />
          ...
          <button className="flex items-center gap-2 bg-slate-900 border border-slate-800 px-4 py-2.5 rounded-xl hover:bg-slate-800 transition-all active:scale-95 group shadow-xl shadow-black/20">
            ...
            <span className="text-[10px] font-black uppercase tracking-widest text-slate-300">PDF Export</span>
          </button>
          <button className="flex items-center gap-2 bg-blue-600 px-6 py-2.5 rounded-xl hover:bg-blue-500 transition-all active:scale-95 group shadow-xl shadow-blue-600/20">
            ...
            <span className="text-[10px] font-black uppercase tracking-widest text-white">Excel Export</span>
          </button>
        </div>
      </section>
      ...
      <section className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 bg-white/[0.03] backdrop-blur-xl border border-white/[0.05] p-10 rounded-[2.5rem] relative overflow-hidden group shadow-2xl shadow-black/40">
          <div className="flex items-center justify-between mb-10 relative z-10">
            <div>
              <h3 className="text-xl font-black text-white uppercase tracking-tight">Sales Trend</h3>
              <p className="text-xs font-bold text-slate-500 uppercase tracking-widest mt-1">Daily transaction volume and revenue</p>
            </div>
            <div className="flex gap-6">
              <span className="flex items-center text-[10px] font-black uppercase tracking-widest text-blue-400">
                <span className="w-2.5 h-2.5 bg-blue-500 rounded-full mr-2 shadow-[0_0_8px_rgba(59,130,246,0.6)]"></span> Revenue
              </span>
              <span className="flex items-center text-[10px] font-black uppercase tracking-widest text-slate-500">
                <span className="w-2.5 h-2.5 bg-slate-600 rounded-full mr-2"></span> Volume
              </span>
            </div>
          </div>
        ...
        <div className="bg-white/[0.03] backdrop-blur-xl border border-white/[0.05] p-10 rounded-[2.5rem] shadow-2xl shadow-black/40">
          <h3 className="text-xl font-black text-white uppercase tracking-tight mb-8">Category Split</h3>
          <div className="space-y-8 mb-10">
            <CategoryBar label="Accessories" percent={45} color="bg-blue-500" />
            <CategoryBar label="Screen Protection" percent={32} color="bg-emerald-500" />
            <CategoryBar label="Spare Parts" percent={18} color="bg-purple-500" />
            <CategoryBar label="Wearables" percent={5} color="bg-amber-500" />
          </div>

          <div className="pt-8 border-t border-white/5">
            <h4 className="text-[10px] font-black text-slate-500 uppercase tracking-[0.3em] mb-8">Payment Velocity</h4>
            <div className="flex items-center justify-around gap-6">
              <PaymentCircle percent={62} label="Card" color="text-blue-500" />
              <PaymentCircle percent={28} label="Cash" color="text-emerald-500" />
              <PaymentCircle percent={10} label="Bank" color="text-purple-500" />
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="bg-white/[0.03] backdrop-blur-xl border border-white/[0.05] rounded-[2.5rem] overflow-hidden shadow-2xl shadow-black/40">
        <div className="p-10 flex justify-between items-center border-b border-white/5 bg-white/5">
          <div>
            <h3 className="text-xl font-black text-white uppercase tracking-tight">Top Performance Index</h3>
            <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest mt-1">Leading products by revenue contribution</p>
          </div>
        ...
        <div className="overflow-x-auto custom-scrollbar">
          <table className="w-full text-left">
            <thead className="bg-slate-900/40 text-slate-500 text-[10px] font-black uppercase tracking-[0.3em]">
              <tr>
                <th className="px-10 py-6">Product & Profile</th>
                <th className="px-8 py-6">Classification</th>
                <th className="px-8 py-6">Volume</th>
                <th className="px-8 py-6">Revenue Index</th>
            ...
            <tbody className="divide-y divide-white/5 text-sm">
              ...
              />
              <ProductRow
                name="Silicon Watch Strap (Active)"
                cat="Accessories"
                sold="850 units"
                revenue="$12,750.00"
                trend="up"
              ...
              />
              <ProductRow
                name="OEM Li-Ion Battery (iPh 13)"
                cat="Spare Parts"
                sold="410 units"
                revenue="$24,600.00"
                trend="down"
    ...
    <tr className="hover:bg-white/[0.02] transition-all group cursor-pointer">
      ...
      <td className="px-8 py-6 uppercase tracking-widest font-black text-[10px]">
        <span className={cn(
          ...
          cat === 'Protection' ? "bg-emerald-500/10 text-emerald-400 border-emerald-500/20" :
          cat === 'Spare Parts' ? "bg-purple-500/10 text-purple-400 border-purple-500/20" :
